...
#[Route('/trip/order', name: 'trip_sheet_')]

class TripOrderController extends AbstractController
{
    ...
    {
        $form = $this->createForm(TripOrderType::class, new TripOrder());
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (TurboBundle::STREAM_FORMAT === $request->getPreferredFormat()) {
                ...
                // отрендерим только block success_stream из того же шаблона
                return $this->renderBlock('gsm/new.html.twig', 'success_stream', [
                    'task' => $task,
                ]);
            }

            // без Turbo всё работает “по-старому”
            return $this->redirectToRoute('task_success', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('gsm/new.html.twig', [
            'form' => $form,
        ]);
    }
}
